...
export class ServiciosClienteComponent implements AfterViewInit, OnInit {
  ...
  seleccionarCorte(corte: PredefinedCut) {
    ...
    this.totalSeleccionado = corte.precio_Total; // actualizar total
  }
...
finish() {
  ...
  const nuevaCita: FinalCite = {
    idCita: 0,
    fecha: this.fechaSeleccionada,
    hora: this.horaSeleccionada,
    precioCorte: this.totalSeleccionado,
    nombreCorte: this.PredefinedCutSeleccionado.nombre.toString(),
    nombreTrabajador: this.trabajadorSeleccionado.nombre.toString()
  };

  this.finalCiteService.guardarCiteService(nuevaCita).subscribe({
    next: (res) => {
