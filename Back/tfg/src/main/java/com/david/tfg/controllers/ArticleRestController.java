...
public class ArticleRestController {
    ...
    @GetMapping("/lista")
    public List<Article> lista() {
        System.out.println("📌 Solicitando lista de artículos");
    ...
    public ResponseEntity<Article> editar(@PathVariable Integer id) {
        Optional<Article> articuloOpt = service.findById(id);
        if (articuloOpt.isPresent()) {
            System.out.println("✅ Artículo encontrado id: " + id);
            return ResponseEntity.ok(articuloOpt.get());
        } else {
            System.out.println("❌ Artículo no encontrado id: " + id);
            return ResponseEntity.notFound().build();
        }
    }
    ...
    public ResponseEntity<Article> guardar(@RequestBody Article articulo) {
        if (articulo.getIdArticulo() == 0) {
            service.save(articulo);
            System.out.println("✅ Artículo creado id: " + articulo.getIdArticulo());
        } else {
            Optional<Article> opt = service.findById(articulo.getIdArticulo());
            if (opt.isPresent()) {
                Article a = opt.get();
                a.setNombre(articulo.getNombre());
                a.setDescripcion(articulo.getDescripcion());
                a.setImagen(articulo.getImagen());
                a.setPrecio(articulo.getPrecio());
                a.setCategoria(articulo.getCategoria());
                service.save(a);
                System.out.println("✅ Artículo actualizado id: " + a.getIdArticulo());
                articulo = a;
            } else {
                System.out.println("❌ No se encontró artículo para actualizar id: " + articulo.getIdArticulo());
                return ResponseEntity.notFound().build();
            }
        }
        return ResponseEntity.ok(articulo);
